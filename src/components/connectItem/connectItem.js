...
import { SETCONNECTUI, SETONUSERIDENTITY } from '../../store/uiStates';
                ...
                <div className='w-[100%] flex justify-between items-center'>
                    <div className='flex flex-col justify-start items-start'>
                        <h3 className='text-md md:text-xl font-semibold md:font-bold flex justify-start items-center w-[9rem] md:w-auto whitespace-nowrap overflow-x-hidden overflow-ellipsis duration-75 hover:underline hover:duration-75 cursor-pointer' onClick={() => {
                            ...
                            dispatch(SETONUSERIDENTITY());
                        }}>Delly SESANGA HIPUNGU <span><CheckBadgeIcon className='text-blueSpecial w-[1.2rem] mt-[2px] md:mt-[4px]' /></span></h3>
                        <p className='text-xs md:text-sm text-darkTextColor mt-[-0.2rem]'>@SesangaDelly</p>
                    </div>
                    <button className='bg-iconsColor text-sm md:text-md text-primary font-semibold rounded-full px-2 md:px-3 py-1 md:py-2 outline-none'>Follow</button>
                </div>
                <p className='leading-[17px] text-sm md:text-md'>Candidat President de la République 2023 sur le projet REFONDATION DU CONGO Avocat, Député et ancien Ministre</p>
            </div>
        </div>
    );
}
